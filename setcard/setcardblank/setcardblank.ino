...
// byte blockcontent[16] = {"1 USE; 1 TRY"};  //an array with 16 bytes to be written into one of the 64 card blocks is defined
...
int checkIfEmpty(byte cardDat[]){
    if(cardDat[1] < 1 && cardDat[2] < 1 && cardDat[3] < 1 && cardDat[4] < 1 && cardDat[5] < 1 && cardDat[6] < 1 && cardDat[7] < 1 && cardDat[8] < 1 && cardDat[9] < 1 && cardDat[10] < 1 && cardDat[11] < 1 && cardDat[12] < 1 && cardDat[13] < 1 && cardDat[14] < 1 && cardDat[15] < 1){
        return 1;
    } else{
        return 0;
    }
